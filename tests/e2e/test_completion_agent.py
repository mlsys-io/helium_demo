...
from helium.frontend.agents.examples import CompletionAgent
...
def run_completion_agent(
    ...
    agent = CompletionAgent(server_config=mock_helium_server.config)
    response = agent.run(
...
def test_completion_agent(
    ...
    run_completion_agent(mock_helium_server, request_config, num_prompts)
