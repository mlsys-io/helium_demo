...
from vllm.vllm.entrypoints.openai.models.protocol import BaseModelPath
from vllm.vllm.entrypoints.openai.models.serving import OpenAIServingModels
from vllm.vllm.entrypoints.utils import get_max_tokens
from vllm.vllm.inputs import PromptType
from vllm.vllm.inputs.data import StreamingInput
...
from vllm.vllm.tokenizers.mistral import (
    MistralTokenizer,
    maybe_serialize_tool_calls,
    truncate_tool_call_ids,
)
from vllm.vllm.utils.async_utils import merge_async_iterators
from vllm.vllm.v1.engine import EngineCoreRequest
from vllm.vllm.v1.engine.async_llm import AsyncLLM
from vllm.vllm.v1.metrics.stats import SchedulerStats

logger = init_child_logger("local")


class SerializedLLMEngine(AsyncLLM):
    """
    ...
    def __init__(self, engine: AsyncLLM | MockLLMEngine) -> None:
        self.engine = engine
    ...
    async def from_engine(
        cls, engine: AsyncLLM | MockLLMEngine
    ) -> AsyncGenerator["SerializedLLMEngine", None]:
    ...
    async def change_kv_role(self, new_role: str) -> None:
        return await self.engine.change_kv_role(new_role)
    ...
    async def get_scheduler_stats(self) -> SchedulerStats:
        return await self.engine.get_scheduler_stats()

    async def clear_scheduler_stats(self) -> None:
        await self.engine.clear_scheduler_stats()
...
class OpenAIEngineClient:
    ...
    async def build(
        ...
        async with engine_context as engine:
            yield await cls.from_serialized_engine(engine, args)
    ...
    async def create_completion(
        ...
        try:
            lora_request = serving_completion._maybe_get_adapters(request)

            tokenizer = serving_completion.renderer.get_tokenizer()

            engine_prompts = await serving_completion._preprocess_completion(
                request, request.prompt, None
            )
        ...
        try:
            for i, engine_prompt in enumerate(engine_prompts):
                ...
                max_tokens = get_max_tokens(
                    serving_completion.max_model_len,
                    request.max_tokens,
                    input_length,
                ...
                if request.use_beam_search:
                    sampling_params = request.to_beam_search_params(
                        max_tokens, serving_completion.default_sampling_params
                    )
                else:
        ...
        model_name = serving_completion.models.model_name(lora_request)
        num_prompts = len(engine_prompts)
        ...
        try:
            ...
            for i, final_res in enumerate(final_res_batch):
                ...
                if final_res.prompt is None:
                    final_res.prompt = engine_prompts[i].get("prompt")  # type: ignore[union-attr]
    ...
    async def create_chat_completion(
        ...
        try:
            lora_request = serving_chat._maybe_get_adapters(request)

            model_name = serving_chat.models.model_name(lora_request)

            tokenizer = serving_chat.renderer.get_tokenizer()
            ...
            (
                ...
                serving_chat.chat_template_content_format,
        ...
        try:
            for i, engine_prompt in enumerate(engine_prompts):
                ...
                if request.use_beam_search:
                    sampling_params = request.to_beam_search_params(
                        max_tokens, serving_chat.default_sampling_params
                    )
                else:
                    sampling_params = request.to_sampling_params(
                        max_tokens,
    ...
    async def clear_scheduler_stats(self) -> None:
        return await self.engine.clear_scheduler_stats()
