...
_METRIC_ALIASES: dict[str, str] = {
    "gpu_prefix_cache_queries": "prefix_cache_queries",
    "gpu_prefix_cache_hits": "prefix_cache_hits",
    "gpu_prefix_cache_hit_rate": "prefix_cache_hit_rate",
}
...
def get_metric_values(
    ...
    for metric in vllm_metrics:
        # Try the canonical name first, then an alias for newer vLLM versions.
        candidates = [metric]
        if metric in _METRIC_ALIASES:
            candidates.append(_METRIC_ALIASES[metric])
        for candidate in candidates:
            if candidate not in metric_map:
                continue
            samples = metric_map[candidate].samples
            if len(samples) > 0:
