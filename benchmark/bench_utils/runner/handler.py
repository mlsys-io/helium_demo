...
from bench_utils.runner.sglang import BenchmarkRunnerWithSGLang
...
def _get_runner(config: RunnerConfig) -> BenchmarkRunner:
    ...
    if system_name in [
        ...
        return BenchmarkRunnerWithSGLang(
            config=config,
            devices=config.devices,
            main_config=main_config,
            worker_configs=None if len(worker_configs) == 1 else worker_configs,
        ...
        return BenchmarkRunnerWithLocalHelium(
            ...
            gpu_util_log_dir=config.gpu_util_log_dir,
            helium_server_config=helium_server_config,
            openai_worker_configs=openai_worker_configs,
        )
    elif system_name == "helium-vllm-controller":
        main_config = BenchVLLMServerConfig(
            model=model_name,
            host=llm_server_config.host,
            port=llm_server_config.port,
        ...
        return BenchmarkRunnerWithLocalHelium(
            ...
            gpu_util_log_dir=config.gpu_util_log_dir,
            helium_server_config=helium_server_config,
            openai_controller_config=main_config,
            openai_worker_configs=worker_configs,
            openai_engine_infos=engine_infos,
        )
