...
class BenchmarkRunnerWithLocalHelium(BenchmarkRunner):
    def __init__(
        ...
        openai_controller_config: BenchVLLMServerConfig | None = None,
        openai_worker_configs: list[BenchVLLMServerConfig] | None = None,
        openai_engine_infos: list[EngineClientInfo] | None = None,
    ) -> None:
        ...
        self.openai_controller_config = openai_controller_config
        self.openai_worker_configs = openai_worker_configs
        self.openai_engine_infos = openai_engine_infos
    ...
    async def start_runner(self) -> None:
        if (
            self.openai_controller_config is not None
            or self.openai_worker_configs is not None
        ):
    ...
    def _start_server(self) -> list[mp.Process]:
        if self.openai_controller_config is not None:
            if self.openai_worker_configs is None or self.openai_engine_infos is None:
                raise ValueError(
                    "Expected worker configs and engine infos when controller config is provided"
                )
            controller_config = self.openai_controller_config.compile([])
            compiled_configs = [
                config.compile([]) for config in self.openai_worker_configs
            ]
        ...
        worker_configs = self.openai_worker_configs
        if worker_configs is None:
            raise ValueError("OpenAI worker configs are not provided")
